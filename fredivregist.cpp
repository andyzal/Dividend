...
ACTION fredivregist::allowance( uint8_t code, name vipname )
...
// After finishing the tests this action may be called only by multisig. 
ACTION fredivregist::removeallow(){
  require_auth( get_self() );
  whitelist_index white_list(get_self(), get_self().value);
...
ACTION fredivregist::proposalvote(  const name voter,
                                    const uint8_t vote )
{
        require_auth(voter);
        ...
        //voter allowed?
        fredivregist::whitelist_index white_list(get_self(), get_self().value);
 ...
 */
     ...
     /*
      ...
      +-----------------------------------
              ...
              +
              */
  ACTION fredivregist::dividcomputed() 
  {
    require_auth(_self);
    register_table registers( get_self(), get_self().value );
...
ACTION fredivregist::zerofortest()
{   
    ...
    /*
     ...
     +---------------------------------
             ...
             +
             */
    ACTION fredivregist::regtransfer()
    {  
      ...
      //Submit leftower tokens from fredivregist to freeosdaodao: 
...
ACTION fredivregist::clear(name user) 
{
 require_auth(get_self());
  whitelist_index white_list(get_self(), get_self().value);
   uint8_t i=0;
...
ACTION fredivregist::votesreset11(name user){
  require_auth(get_self());
  whitelist_index white_list(get_self(), get_self().value);
   uint8_t i=0;
   for( auto iter=white_list.begin(); iter!=white_list.end(); iter++ ){
...
void fredivregist::prop_reset() {  //erase content of proposal 
  proposal_table proposals(get_self(), get_self().value);
  require_auth(get_self());
  auto pro_itr = proposals.find(1);
  proposals.modify(pro_itr, get_self(), [&](auto &p) {
...
ACTION fredivregist::regchown(name userfrom, name userto, uint64_t nft_id){
